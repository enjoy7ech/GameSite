...
class PolyPiece {
    ...
    drawImage(special) {
        ...
        // make shadow
        ctx.fillStyle = "none";
        ctx.shadowColor = this.isMergeHighlight 
            ? "rgba(212, 175, 55, 0.9)" 
            : (this.isItemHighlight && this.rot === 0)
                ? "rgba(255, 215, 0, 0.85)" // Golden Spiritual Glow
                : this.selected
                    ? special
                        ? "rgba(252, 186, 3, 0.9)" // Amber for correct
                        : "rgba(225, 192, 100, 0.7)" // Silk Gold for selection
                    : "transparent";
        ctx.shadowBlur = (this.isMergeHighlight || this.isItemHighlight) ? 25 : 10;
        
        const isEffectActive = this.selected || (this.isItemHighlight && this.rot === 0);
        
        ctx.shadowBlur = isEffectActive ? 10 : 4;
        ctx.shadowOffsetX = isEffectActive ? 0 : -4;
        ctx.shadowOffsetY = isEffectActive ? 0 : 4;

        ctx.fill(pth);
        if (isEffectActive) {
            ctx.fill(pth); // Only one extra fill for a cleaner highlight
        }
    ...
    isPointInPath(p) {
        if (!this.playPath) {
            // Regeneration only, direct use of existing fromSrcMatrix to prevent shifting
            this.playPath = new Path2D();
            this.playPath.addPath(this.srcPath, this.fromSrcMatrix);
        }
...
class Puzzle {
    ...
    constructor(params) {
        ...
        this.container.addEventListener("mousedown", (event) => {
            ...
            if (event.button != 0 && event.button != 2) return; //only left and right button involved
            events.push({
                event: "touch",
        ...
        this.container.addEventListener("mouseup", (event) => {
            useMouse = true;
            event.preventDefault();
            if (event.button != 0 && event.button != 2) return; // ignore if not left or right click
        ...
        function handleLeave(event) {
            events.push({ event: "leave", shiftKey: event.shiftKey, button: event.button }); //
        }
    } // Puzzle
...

{
    // scope for animate
    let state = 0;
    let moving = window._movingState = {}; // for information about moved piece
    let tmpImage;
    let tInit;
    let filesave;
    ...
    animate = function (tStamp) {
        ...
        switch (state) {
                ...
                switch (event.event) {
                    ...
                    case "leave":
                        if (puzzle.rotationStep && tStamp < moving.tInit + 250) {
                            ...
                            if (event.shiftKey || event.button === 0 || event.button === undefined)
                                // left click or touch: turn ccw (left)
                                moving.pp.rotate(
                                    (moving.pp.rot + puzzle.nbRot - 1) % puzzle.nbRot
                                );
                            else 
                                // right click: turn cw (right)
                                moving.pp.rotate((moving.pp.rot + 1) % puzzle.nbRot);
                        }
                        ...
                        do {
                            ...
                            for (let k = puzzle.polyPieces.length - 1; k >= 0; --k) {
                                ...
                                if (moving.pp.ifNear(pp)) {
                                    ...
                                    if (puzzle.onMerge && !puzzle.disableMergeEvent) puzzle.onMerge();
                                    doneSomething = true;
                                    break;
                                }
                            } // for k
            ...
            case 60: // winning
                ...
                tmpImage.style.left = `${((puzzle.polyPieces[0].x + (puzzle.srcWidth * puzzle.scale) / 2) /
                        puzzle.contWidth) *
                    100
                    }%`;
                tmpImage.style.top = `${((puzzle.polyPieces[0].y + (puzzle.srcHeight * puzzle.scale) / 2) /
                        puzzle.contHeight) *
                    100
                    }%`;
                tmpImage.style.boxShadow = "-4px 4px 4px rgba(0, 0, 0, 0.5)";
