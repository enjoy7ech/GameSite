...
export function spawnNewBall() {
    ...
    const minWidth = 4 + factor * 6;
    const maxWidth = 12 + factor * 18;
    ...
    if (state.gameMode === 'rush') {
        ...
        // --- 下调无尽关（高factor下）的距离上限，避免球实在太远 ---
        const minDepth = 4 + factor * 6;
        const maxDepth = 14 + factor * 14; 
    ...
    for (let ball of state.activeBalls) {
        ...
        if (distToHoop < RING_RADIUS * 0.9) {
            ...
            if (bPos.y < HOOP_POS.y && bPos.y > HOOP_POS.y - 1.2 && ball.enteredRim) {
                ...
                // 当单次难度分达到 999 后直接获胜结束游戏
                if (difficultyPts >= 999) {
                    state.achieved999 = true;
                    endGame(true);
                    return;
                }
...
export function endGame(win) {
    ...
    if (state.achieved999) {
        statusEl.innerText = "神之领域";
        msgEl.innerText = "SYSTEM OVERRIDE // 999 难度突破，物理法则已重写。";
        statusEl.style.background = "linear-gradient(90deg, #ff00ea, #ffffff, #00fff2)";
        statusEl.style.webkitBackgroundClip = "text";
        statusEl.style.webkitTextFillColor = "transparent";
        statusEl.style.animation = "glitch 1s infinite alternate";
